...
export default function DashboardPage() {
    ...
    // FIXED: This function now reads balance from user document, not from orders
    const fetchUserData = async (user: any) => {
        ...
        if (userDoc.exists()) {
            ...
            // IMPORTANT: Read balance from user document (this is the source of truth)
            const currentBalance = userData.totalEarnings || 0;
            setTotalEarnings(currentBalance);

            // Get referral count (how many people used your link)
        ...
        // Fetch user's own orders (what they bought)
        ...
        // Fetch products for referral links
        const productsSnapshot = await getDocs(collection(db, 'products'));
        setProducts(productsSnapshot.docs.map(doc => ({ id: doc.id, ...doc.data() })) as Product[]);
    };

    useEffect(() => {
        // Check if returning from withdrawal
                    ...
                    <div className="bg-gradient-to-r from-green-600 to-green-700 text-white rounded-2xl shadow-lg p-6 mb-8">
                        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                            ...
                            <Link
                                ...
                                className={`px-6 py-3 rounded-xl font-semibold transition ${totalEarnings >= 150
